...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="flex flex-wrap gap-2 mb-8" data-tab-group="subjects">
        @foreach($subjects as $subject)
        <button data-tab="{{ $subject }}"
                class="px-5 py-2 rounded-full border font-semibold text-sm transition-all
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            ...
            <a href="{{ route('grade.show', $class->slug) }}"
               class="card-hover bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
                ...
                @if($class->teacher)
                <p class="text-slate-500 text-xs mb-1"><i class="fa-solid fa-chalkboard-user text-navy-300 mr-1"></i>{{ $class->teacher }}</p>
                @endif
                ...
                <div class="mt-3 pt-3 border-t border-slate-100 space-y-1">
                    ...
                    <p class="text-xs text-slate-500"><i class="fa-regular fa-calendar text-navy-300 mr-1"></i>{{ $s->day }} {{ \Carbon\Carbon::parse($s->start_time)->format('h:i A') }}</p>
                    @endforeach
                </div>
                @endif
            </a>
