...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
 ...
 <div class="bg-gradient-to-br from-navy-800 to-navy-950 h-40 flex items-center justify-center relative overflow-hidden">
    ...
    <img src="{{ asset('storage/' . $img) }}"
         alt="{{ $course->title }}"
         class="absolute inset-0 w-full h-full object-cover opacity-80 lightbox-trigger"
         onerror="this.style.display='none'">
    <div class="absolute inset-0 bg-navy-950/40 pointer-events-none"></div>
    @endif
    <i class="fa-solid fa-book-open text-gold-400 text-5xl relative z-10 {{ $img ? 'opacity-0' : '' }} pointer-events-none"></i>
    @if($course->is_featured)
    <span class="absolute top-3 right-3 bg-gold-500 text-navy-900 text-xs font-bold px-2.5 py-1 rounded-full z-10 pointer-events-none">Featured</span>
    @endif
</div>
            <div class="p-5">
                @if($course->category)
